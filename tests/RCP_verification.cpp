...
void make_model(TFE_model& model, const std::array<float, 3> dimentions, const std::array<int, 3> mesh)
{
    ...
    // Создание узлов
    for (int step_y = 0; step_y < ly / dy + 1; ++step_y)
    {
        y = step_y * dy;
        for (int step_z = 0; step_z < lz / dz + 1; ++step_z)
        {
            z = step_z * dz;
            for (int step_x = 0; step_x < lx / dx + 1; ++step_x)
            {
                x = step_x * dx;
    ...
    for (const auto& node : model.Nodes())
    {
        ...
        if (node.coords().y == ly) {break;}
        if ((node.coords().x == lx) || (node.coords().z == lz)) {continue;}
